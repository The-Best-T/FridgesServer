...
{
    public class ValidateFridgeModelExistsAttributeTests
    {
        ...
        public async Task OnActionExecutionAsync_TestResult_GoToAction()
        {
            ...
            _repositoryMock.Setup(rp => rp.FridgeModel.GetFridgeModelAsync(It.IsAny<Guid>(), It.IsAny<bool>()))
                .Returns(Task.FromResult(new FridgeModel()));

            var controller = new ValidateFridgeModelExistsAttribute(null, _repositoryMock.Object);

            var (context, next) = GetContextAndDelegate(controller);

            context.ActionArguments.Add("fridgeModelId", It.IsAny<Guid>());
        ...
        public async Task OnActionExecutionAsync_TestHttpItems_SetFridge()
        {
            ...
            //Asser
            Assert.NotNull(context.HttpContext.Items["fridgeModel"]);
        }
        ...
        public async Task OnActionExecutionAsync_TestResult_NotFound()
        {
            ...
            _repositoryMock = new Mock<IRepositoryManager>();
            _repositoryMock.Setup(rp => rp.FridgeModel.GetFridgeModelAsync(It.IsAny<Guid>(), It.IsAny<bool>()))
                .Returns(Task.FromResult((FridgeModel)null));

            _loggerMock = new Mock<ILoggerManager>();
            _loggerMock.Setup(l => l.LogInfo(It.IsAny<string>()));

            var controller = new ValidateFridgeModelExistsAttribute(_loggerMock.Object, _repositoryMock.Object);
        ...
        private (ActionExecutingContext, ActionExecutionDelegate) GetContextAndDelegate(ValidateFridgeModelExistsAttribute controller)
        {
